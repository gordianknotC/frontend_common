...
export type AllowedLoggers<M extends string> = Record<M, AllowedModule<M>>;
...
function message(moduleName: string, level: ELevel, msg: any[], traceBack: number = 2, stackNumber: number = 5): LogRecord {
...
export class Logger<M> implements LoggerMethods{
  ...
  private static allowedModules: AllowedLoggers<any> = {} as any;
  ...
  static setLoggerAllowance<M extends string>(
    option: Partial<AllowedLoggers<M>>
  ) {
  ...
  constructor(
    option: Pick<AllowedModule<M>, "moduleName">
  ) {
